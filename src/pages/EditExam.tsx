...
export default function EditExam() {
  ...
  const fetchExamData = async () => {
    ...
    try {
      ...
      // Fetch answers separately (as admin can access question_answers)
      const { data: answers, error: aError } = await supabase.from('question_answers').select('*');
      ...
      if (exam.category === "MCQ") {
        setMcqQuestions(questions.map(q => {
          ...
          return {
            ...
            question: q.question,
            options: q.options,
            correctAnswer: answer?.correct_answer !== undefined ? answer.correct_answer : 0,
            points: q.points
          };
        }));
      } else {
  ...
  const handleUpdate = async (status: 'active' | 'draft') => {
    ...
    try {
      ...
      // 2. Update existing questions and their answers (safer than delete + insert)
      if (category === "MCQ") {
        ...
        const incomingIds = mcqQuestions.filter(q => q.db_id).map(q => q.db_id);

        // Delete questions that are no longer in the edit form
        ...
        for (const q of mcqQuestions) {
          if (q.db_id) {
            ...
            // Update correct answer (don't delete, just update)
            ...
            // Insert answer for new question
        ...
        const existingIds = existingQuestions?.map(q => q.id) || [];
        const incomingIds = dsaQuestions.filter(q => q.db_id).map(q => q.db_id);
        ...
        for (const q of dsaQuestions) {
          if (q.db_id) {
            ...
            // Insert test cases for new question
            await supabase.from('question_answers').insert({
              question_id: qData.id,
              test_cases: q.testCases
            });
