...
BEGIN
  ...
  FOR v_q_record IN SELECT id, points, type FROM questions WHERE exam_id = p_exam_id LOOP
    ...
    v_is_correct := FALSE;
    v_points := 0;
    v_ans_record := NULL;  -- Reset record for each iteration to prevent stale data
    ...
    IF v_ans_record.correct_answer IS NOT NULL AND (p_answers->>(v_q_record.id::text))::integer = v_ans_record.correct_answer THEN
